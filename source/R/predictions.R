...
# -------------------------------------------------------------------------
# linear regression model with log-transformations of y & x ---------------

#' @title predict_linreg_logtrafo
#' @param draws_df data.frame with posterior draws
#' @param new_data data.frame with new observations to make predictions for
#' @param level Uncertainty level for the prediction interval, default = .95
predict_linreg_logtrafo <- function(draws_df, new_data) {
  stopifnot(# should be the same!
    length(new_data) == length(unique(draws_df$.rep))
  )
  ...
  preds <- draws_list %>%
    mutate(
      preds = purrr::map2(draws, validation_data, predx_linreg_logtrafo)
    ) %>%
    select(.rep, preds) %>%
    unnest(preds)
  return(preds)
...
#' @title predx_linreg_logtrafo
#' @description
#' Actual prediction function for the linear models
#' @param draws ....
#' @param newdata ....
#' @param level ....
predx_linreg_logtrafo <- function(draws, newdata, level = .95) {
  # linear predictor
  eta <- expand_grid(draws, newdata) %>%
    mutate(eta = b_Intercept + b_x_obs * log(x_obs))
  ...
  yhat <- eta %>%
    ...
    summarise(
      .by     = uniqueid, 
      yhat    = mean(exp(eta)),
      yhat_ll = quantile(exp(ydraw), (1 - level) / 2),
      yhat_ul = quantile(exp(ydraw), 1 - (1 - level) / 2)
    )
...
#' @title predict_gammareg
#' @param draws_df data.frame with posterior draws
#' @param new_data data.frame with new observations to make predictions for
#' @param level Uncertainty level for the prediction interval, default = .95
predict_gammareg <- function(draws_df, new_data) {
  stopifnot(# should be the same!
    length(new_data) == length(unique(draws_df$.rep))
  )
  ...
  preds <- draws_list %>%
    mutate(
      preds = purrr::map2(draws, validation_data, predx_gammareg)
    ) %>%
    select(.rep, preds) %>%
    unnest(preds)
  return(preds)
...
#' @title predx_gammareg
#' @description
#' Actual prediction function for the gamma GLM
#' @param draws ....
#' @param newdata ....
#' @param level ....
predx_gammareg <- function(draws, newdata, level = .95) {
  # linear predictor
  eta <- expand_grid(draws, newdata) %>%
    mutate(eta = beta0 + beta1 * log(x_obs))
  
  # posterior point prediction & prediction interval
  yhat <- eta %>%
    mutate(ydraw = rgamma(n(), shape = shape, rate = shape / exp(eta))) %>%
    summarise(
      .by     = uniqueid, 
      yhat    = mean(exp(eta)),
      yhat_ll = quantile(ydraw, (1 - level) / 2),
...
# -------------------------------------------------------------------------
# Gamma GLM with known CV -------------------------------------------------

#' @title predx_gamma_eivknowncv
#' @description
#' Actual prediction function for the gamma GLM with known 
#' CV of the measurement error
#' @param draws ....
#' @param newdata ....
#' @param level ....
predx_gamma_eivknowncv <- function(draws, newdata, level = .95) {
