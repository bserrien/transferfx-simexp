...
rm(tidy_scenario, reg_dilution, scenario_labeller_gamma, simexp_design5, 
   simexp_design6, simexp_design7)

labs <- colnames(simexp_design5bis)[grepl("_label", 
                                          colnames(simexp_design5bis))]
...
list(
  ...
  tar_quarto(
    report_simexp5bis,
    path = here("source/quarto/analysis-simexp5bis.qmd")
  )
)
